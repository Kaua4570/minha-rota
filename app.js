...
(() => {
  ...
  function buildChartSeries(period) {
    ...
    if (period === 'dia') {
      ...
      for (let i = 13; i >= 0; i--) {
        ...
        const t = state.days[key] ? getTotals(key) : { kmRodado: 0, lucro: 0, combustivel: 0 };
        points.push({
    ...
    if (period === 'semana') {
      ...
      return weeks.map(({ start, end }) => {
        let km = 0, lucro = 0, combustivel = 0;
        const cursor = new Date(start);
        while (cursor <= end) {
          const key = todayKey(cursor);
          if (state.days[key]) {
        ...
        return { label, km, lucro, combustivel };
      });
    }
  ...
  function renderChartCanvas(points) {
    ...
    // Rótulos do eixo X (mostra no máximo ~6 para não poluir).
